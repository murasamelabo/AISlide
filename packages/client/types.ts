...
export type PreviewOptions = { page_indices?: number[] | null; max_dimension?: number; layout?: 'pages' | 'contact_sheet'; max_output_bytes?: number }
export type PreviewImage = { base64: string; mime_type: 'image/png'; width: number; height: number; byte_length: number; sha256: string }
export type PresentationPreview = { revision: number; hash: string; pages: { page_index: number; slide_id: string; image_index: number; x: number; y: number; width: number; height: number }[]; images: PreviewImage[]; warnings: StaticExport['warnings']; office_visual_parity: false }
export type PreflightOptions = { page_indices?: number[] | null; min_font_size?: number }
export type PreflightFinding = { code: string; severity: 'warning' | 'error'; evidence: 'renderer' | 'geometry' | 'heuristic' | 'document'; page_index: number; slide_id: string; element_ids: string[]; scopes: string[]; bounds: { x: number; y: number; width: number; height: number }; message: string; suggestions: string[] }
