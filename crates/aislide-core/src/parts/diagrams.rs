    ...
    match category {
        ...
        _ => {
            ...
            match category {
                ...
                "cycle" | "radiation" => radial(drawing,items,center,category=="cycle",variant),
...
fn flow(drawing: &mut Drawing, items: &[PartItem], vertical: bool, variant: usize) -> Result<()> {
    ...
    if vertical {
        ...
        for (index,item) in items.iter().enumerate() {
            let top=90.0+index as f64*row_height;
            if index+1<items.len() { drawing.line([variant as f64*32.0+80.0,top+34.0],[variant as f64*32.0+80.0,top+row_height+12.0],variant==0,"@dk2"); }
            drawing.shape(if variant==2 {"diamond"} else {"ellipse"},[58.0+variant as f64*32.0,top,44.0,44.0],"@lt1",&accent(index));
            drawing.text(&(index+1).to_string(),[62.0+variant as f64*32.0,top+7.0,36.0,30.0],18.0,"@dk1",true,TextAlign::Center);
            drawing.text(&item.label,[200.0,top,300.0,row_height-14.0],22.0,"@dk1",true,TextAlign::Left);
            drawing.text(&item.detail,[532.0,top+4.0,548.0,row_height-14.0],18.0,"@dk2",false,TextAlign::Left);
            if variant==2 { drawing.rect([200.0,top+row_height-12.0,880.0,1.0],"@lt2"); }
        }
    } else {
        let slot=1104.0/items.len() as f64;
        for (index,item) in items.iter().enumerate() {
            let left=24.0+index as f64*slot; let width=slot-30.0;
            if index+1<items.len() { drawing.line([left+width,244.0],[left+slot-5.0,244.0],true,"@dk2"); }
            if variant==1 { drawing.shape("chevron",[left,152.0,width,180.0],"@lt2",&accent(index)); }
            else if variant==0 { drawing.rect([left,152.0,width,180.0],"@lt2"); }
            else { drawing.line([left,158.0],[left+width,158.0],false,"@dk2"); drawing.shape("ellipse",[left,140.0,36.0,36.0],"@lt1",&accent(index)); }
            drawing.text(&format!("{:02}",index+1),[left+22.0,variant as f64*4.0+188.0,width-44.0,36.0],24.0,&accent(index),true,TextAlign::Left);
            drawing.text(&item.label,[left+22.0,240.0,width-44.0,64.0],20.0,"@dk1",true,TextAlign::Left);
            drawing.text(&item.detail,[left+8.0,362.0,width-16.0,100.0],16.0,"@dk2",false,TextAlign::Left);
        }
    }
    Ok(())
}
...
fn tree(drawing: &mut Drawing, variant: usize, data: &PartData) -> Result<()> {
    ...
    let depth=|index:usize| { let mut current=&nodes[index]; let mut level=0; while let Some(parent)=&current.parent { current=nodes.iter().find(|node| &node.id==parent).expect("validated parent"); level+=1; } level };
    let levels:Vec<_>=(0..nodes.len()).map(depth).collect(); let maximum=levels.iter().copied().max().unwrap_or(0);
    let positions:Vec<_>=nodes.iter().enumerate().map(|(index,_)| {
        let level=levels[index]; let peers=levels.iter().filter(|entry| **entry==level).count(); let order=levels[..index].iter().filter(|entry| **entry==level).count();
        if variant==2 {[56.0+level as f64*72.0,100.0+index as f64*384.0/nodes.len() as f64]}
        else if variant==1 {[72.0+level as f64*840.0/(maximum+1) as f64,118.0+(order as f64+0.5)*344.0/peers as f64]}
        else {[32.0+(order as f64+0.5)*1088.0/peers as f64,112.0+level as f64*320.0/maximum.max(1) as f64]}
    }).collect();
    for (index,node) in nodes.iter().enumerate() { if let Some(parent)=&node.parent { let parent=nodes.iter().position(|node| &node.id==parent).expect("validated parent"); drawing.line(positions[parent],positions[index],false,"@dk2"); } }
    for (index,node) in nodes.iter().enumerate() { let position=positions[index]; let width=if variant==2 {720.0} else {184.0}; let left=if variant==0 {position[0]-width/2.0} else {position[0]}; let top=if variant==2 {position[1]} else {position[1]-24.0}; let height=if variant==2 {28.0} else {60.0};
        drawing.rect([left,top,width,height],"@lt1"); if variant!=2 {drawing.rect([left,top,3.0,height],&accent(levels[index]));}
        drawing.text(&node.label,[left+12.0,top+if variant==2 {0.0} else {8.0},width-24.0,height-8.0],18.0,"@dk1",true,TextAlign::Left);
    }
    Ok(())
}
...
fn network(drawing: &mut Drawing, variant: usize, data: &PartData) -> Result<()> {
    ...
    for (index,edge) in edges.iter().enumerate() { let from=positions[edge.from];let to=positions[edge.to]; drawing.line(from,to,false,"@dk2"); let middle=[(from[0]+to[0])/2.0,(from[1]+to[1])/2.0]; drawing.rect([middle[0]-64.0,middle[1]-12.0,128.0,26.0],"@lt1"); drawing.text(&edge.label,[middle[0]-62.0,middle[1]-10.0,124.0,24.0],13.0,&accent(index),false,TextAlign::Center); }
...
fn venn(drawing: &mut Drawing, items: &[PartItem], center: &str, variant: usize) -> Result<()> {
    ...
    drawing.text(center,if variant==1 {[514.0,258.0,76.0,58.0]} else {[500.0,260.0,120.0,72.0]},16.0,"@dk1",true,TextAlign::Center);
    Ok(())
}
