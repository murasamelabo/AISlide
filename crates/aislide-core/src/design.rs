...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Master { pub id: String, pub name: String, pub background: String, pub elements: Vec<Element> }
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
fn placeholder(id: &str, kind: PlaceholderKind, index: u32, bounds: [f64; 4], size: f64, text: &str) -> Element {
    Element::Text { id: id.into(), x: bounds[0], y: bounds[1], width: bounds[2], height: bounds[3], text: text.into(), font_size: size, color: "@dk1".into(), bold: kind == PlaceholderKind::Title,
...
impl Default for Design {
    fn default() -> Self {
        ...
        Self { theme: Theme::default(), masters: vec![Master { id: "master-1".into(), name: "Main master".into(), background: "@lt1".into(), elements: Vec::new() }], layouts: vec![
...
pub fn validate_design(design: &Design) -> Result<()> {
    ...
    for master in &design.masters {
        ...
        crate::model::validate_elements(&master.elements, (1280.0, 720.0), 0, &mut BTreeSet::new(), &mut total, &mut images)?;
    }
    ...
    for layout in &design.layouts {
        ...
        crate::model::validate_elements(&layout.elements, (1280.0, 720.0), 0, &mut BTreeSet::new(), &mut total, &mut images)?;
...
fn inherit(existing: &Element, template: &Element) -> Element {
    match (existing, template) {
        (Element::Text { id, text, .. }, Element::Text { .. }) => {
            let mut result = template.clone();
            if let Element::Text { id: new_id, text: new_text, format, .. } = &mut result { *new_id = id.clone(); *new_text = text.clone(); format.inherit_layout = true; }
            result
        }
        _ => existing.clone(),
    }
...
pub fn update_design(mut deck: Deck, design: Design) -> Result<Deck> {
    ...
    deck.design = Some(design); validate_deck(&deck)?; Ok(deck)
}

pub fn assign_layout(mut deck: Deck, slide_id: &str, layout_id: &str) -> Result<Deck> {
    ...
    for template in &layout.elements {
        if let Element::Text { format, .. } = template {
            ...
            if let Some(existing) = slide.elements.iter_mut().find(|existing| existing.bounds().0 == template.bounds().0 || matches!(existing, Element::Text { format: other, .. } if other.placeholder == format.placeholder)) { *existing = inherit(existing, template); }
            else { slide.elements.push(inherit(template, template)); }
        }
    }
    for element in &mut slide.elements {
        if let Element::Text { format, .. } = element {
...
pub fn apply_theme(mut deck: Deck, theme: Theme) -> Result<Deck> {
    ...
    for slide in &mut deck.slides { bind(&mut slide.background); visit(&mut slide.elements, &bind); }
    let design = deck.design.get_or_insert_with(Design::default); design.theme = theme;
    for master in &mut design.masters { bind(&mut master.background); visit(&mut master.elements, &bind); }
    for layout in &mut design.layouts { if let Some(background) = &mut layout.background { bind(background); } visit(&mut layout.elements, &bind); }
    validate_deck(&deck)?; Ok(deck)
}
