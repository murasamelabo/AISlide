...
pub fn create_diagram(id: &str, steps: &[String]) -> Result<Element> {
    ...
    for (index, step) in steps.iter().enumerate() {
        ...
        children.push(Element::Text { id: format!("{id}-text-{index}"), x: left + 16.0, y: 76.0, width: width - 32.0, height: 130.0, text: step.clone(), font_size: 22.0, color: "202525".into(), bold: true });
        if index + 1 < steps.len() {
            children.push(Element::Connector { id: format!("{id}-link-{index}"), x: left + width, y: 141.0, width: gap, height: 1.0, color: "586563".into(), stroke_width: 2.0, arrow: true, flip_v: false,
                start: Some(Connection { element_id: shape_id, site: 3 }), end: Some(Connection { element_id: format!("{id}-node-{}", index + 1), site: 1 }) });
        }
    }
