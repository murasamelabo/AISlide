...
export class McpAssets {
  ...
  retain(bytes, format, name) { return this.retainMany([{ bytes, format, name }])[0]; }
  retainMany(inputs) {
    const staged = new Map(this.#assets);
    let totalBytes = this.#bytes;
    const result = inputs.map(({ bytes, format, name }) => {
      ...
      const asset = Object.freeze({ asset_id: randomUUID(), name, format, mime_type: definition[0], byte_length: bytes.length, sha256, base64: bytes.toString('base64') });
  ...
  async registerFile({ path, root: rootIndex = 0 }, signal) {
    ...
    try {
      ...
      signal?.throwIfAborted();
      return this.retain(buffer.subarray(0, length), format, basename(path));
