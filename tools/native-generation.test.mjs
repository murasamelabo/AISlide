...
test('native WebView authoring and generation use shared edits, inheritance and undo', { timeout: 90_000, skip: process.platform !== 'win32' }, async (context) => {
  ...
  const ribbonBounds = await page.locator('.ribbon').boundingBox();
  assert.ok(tooltipBounds.y + tooltipBounds.height > ribbonBounds.y + ribbonBounds.height, 'The tooltip must extend outside the scrolling ribbon');
