...
New parts use theme-linked native objects at `{x:64,y:144,width:1152,height:512}`. Measured text shrinks only to 12px; input that cannot fit is rejected. Chart values remain raw in embedded XLSX, including percentage stacks. This is deterministic compilation, not model inference.
...
- Node: `{id,label,kind?,x,y,width?,height?,fill?,stroke?,color?,font_size?,group?}`. Kinds: `rectangle`, `rounded_rectangle`, `ellipse`, `diamond`, `cylinder`, `cloud`. Font size 12-40, default 18; colors accept RGB/theme references. Default fill/outline/text: `@lt1`/`@accent1`/`@dk1`.
...
`object_catalog` and `design_defaults` are read-only MCP tools. `add_object`, `update_design`, `apply_theme`, and `assign_layout` require `deck_id` and `expected_revision`, plus their operation-specific inputs. `add_object` also requires `slide_id`. `update_text` accepts top-level text boxes and preset-shape text. All these mutations are undoable and reject stale revisions.
