...
Limits: 1-48 nodes, 64 edges, 8 boundaries; 1152x512 below an 88px title band; the rendered slide remains limited to 256 elements. Shapes have separate label text within the root group. PowerPoint shape movement keeps connectors attached but does not move the separate label automatically. Graph-editor movement moves labels and recomputes connections together.
